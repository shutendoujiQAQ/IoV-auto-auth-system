...
'''def create_UrbanSound8K_list(audio_path, metadata_path, list_path):
    ...
    for i, line in enumerate(lines):
        ...
        sound_path = os.path.join(audio_path, f'fold{data[3]}', data[0]).replace('\\', '/')
        if sound_sum % 10 == 0:
            f_test.write(f'{sound_path}\t{data[4]}\n')
        else:
            f_train.write(f'{sound_path}\t{data[4]}\n')
        sound_sum += 1
...
def create_UrbanSound8K_list(audio_path, metadata_path, list_path):
    ...
    for i, line in enumerate(lines):
        ...
        class_id = int(data[4])
        if class_id not in labels.keys():
            labels[class_id] = data[5]
        sound_path = os.path.join(audio_path, f'fold{data[3]}', data[0]).replace('\\', '/')
        if sound_sum % 10 == 0:
            f_test.write(f'{sound_path}\t{class_id}\n')
        else:
            f_train.write(f'{sound_path}\t{class_id}\n')
        sound_sum += 1
...
if __name__ == '__main__':
    ...
    #                                       list_path='dataset/')
    # 创建UrbanSound8K数据列表
    create_UrbanSound8K_list(audio_path='dataset_myproject/Trafic/audio',
                             metadata_path='dataset_myproject/Trafic/metadata/Trafic.csv',
                             list_path='dataset_myproject')
