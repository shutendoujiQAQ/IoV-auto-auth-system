...
class XboxController(object):
    ...
    def __init__(self, world):
        ...
        if isinstance(world.player, carla.Vehicle):
            ...
            self._reverse = False
            self._overtake_light = False  # 默认超车灯关闭
        else:
            raise NotImplementedError("仅支持车辆控制")
        self._reverse = False
        # 设置每 0.5 秒自动更新 BUSDATA.json 文件
    ...
    def update_busdata(self):
        ...
        if self.world.player is not None:
            ...
            speed = math.sqrt(self.world.player.get_velocity().x**2 +
                              self.world.player.get_velocity().y**2 +
                              self.world.player.get_velocity().z**2) * 3.6  # km/h
            angle = steer_axis * 60  # 转向角度，假设最大角度为 40°
    ...
    def get_vehicle_data(self):
        ...
        speed = math.sqrt(self.world.player.get_velocity().x**2 +
                          self.world.player.get_velocity().y**2 +
                          self.world.player.get_velocity().z**2) * 3.6  # 转换为 km/h
        angle = steer_axis * 60  # 最大转向角度为 40°
...
class HUD(object):
    def __init__(self, width, height):
        ...
        self.json_path = os.path.join(script_dir, "result.json")
        # 如果文件不存在就先创建一个默认值
        ...
        print(f"[HUD Init] json_path = {self.json_path}")
        # 新增：保存最近一次车辆控制信息
    ...
    def render(self, display, controller):
        ...
        # 显示实时油门、刹车、车速和转向
        text_throttle = self.font.render(f"Throttle: {throttle:.2f}", True, (255, 255, 255))
        text_brake = self.font.render(f"Brake: {brake:.2f}", True, (255, 255, 255))
        text_speed = self.font.render(f"Speed: {speed:.1f} km/h", True, (255, 255, 255))
        text_angle = self.font.render(f"Steering: {angle:.1f}°", True, (255, 255, 255))

        # 设置文本的位置（在黄灯和红灯下方）
        display.blit(text_throttle, (10, 150))  # Adjust the position as necessary
        display.blit(text_brake, (10, 180))
        display.blit(text_speed, (10, 210))
        display.blit(text_angle, (10, 240))
          # 在 HUD 上显示场景数据，并进行编码转换
        scene_text = self.font.render(f"Scene: {vlm['scene_class']}", True, (255, 255, 255))

        # 为防止乱码，确保文本以 UTF-8 编码显示
        try:
            scene_text = self.font.render(f"Scene: {vlm['scene_class']}".encode('utf-8').decode('utf-8'), True, (255, 255, 255))
        except UnicodeEncodeError:
            scene_text = self.font.render(f"Scene: Error", True, (255, 255, 255))

        display.blit(scene_text, (10, 270))
    ...
    def render(self, display):
        ...
        if self.json_info_text:
            font = pygame.font.Font(None, 28)
            text_surface = font.render(self.json_info_text, True, (255, 255, 255))
            text_bg_rect = text_surface.get_rect()
            text_bg_rect.topleft = (10, 50)
            pygame.draw.rect(display, (0, 0, 0), text_bg_rect.inflate(10, 10))
            display.blit(text_surface, (15, 55))
...
def game_loop(args):
    ...
    try:
        ...
        while True:
            ...
            # 读取子进程推理结果并在 HUD 上弹通知
            try:
                while True:
                    fold, cls = result_queue.get_nowait()
                    text = f"Audio infer: {fold} → class={cls}"
                    hud.notification(text, seconds=3.0)
            except queue.Empty:
                pass
            # 处理手柄及其他事件
...
def main():
    ...
    parser.add_argument('--rolename',default='hero',help='actor role')
    parser.add_argument('--gamma',default=2.2,type=float,help='Gamma correction')
