...
def scene_normal_low(vlm, dl, bus):
    if vlm.get('special_case') != 0: return 0.0
    return fuzzy_low(bus.get('speed', 0), 0, 80)

def scene_normal_high(vlm, dl, bus):
    if vlm.get('special_case') != 0: return 0.0
    return fuzzy_high(bus.get('speed', 0), 80, 120)

def scene_hard_brake(vlm, dl, bus):
    thr = bus.get('throttle', 0)
    brk = bus.get('brake', 0)
    return min(fuzzy_low(thr, 0, 0.1), fuzzy_high(brk, 0.7, 1.0))
...
def scene_traffic_jam(vlm, dl, bus):
    if vlm.get('special_case') != 3: return 0.0
    return fuzzy_low(bus.get('speed', 0), 0, 20)
...
def rule_low_visibility(vlm, dl, bus, req):
    if vlm.get('low_visibility') == 1:
        req['spd'] = min(req['spd'] + 20, 150)

SPECIAL_RULES = [rule_low_visibility]
...
def choose_methods(req):
    ...
    if opt.check() != sat:
        print(f"当前阈值组合下无满足的验证方法，safe 阈值: {req['safe']}, spd 阈值: {req['spd']}")
        with open('no_matching_methods_log.txt', 'a', encoding='utf-8') as f:
            timestamp = time.strftime("%Y-%m-%d %H:%M:%S")
            f.write(f"{timestamp} - 当前阈值组合下无满足的验证方法，safe 阈值: {req['safe']}, spd 阈值: {req['spd']}\n")
...
# ------------------------------------------------------------------
# 主程序：持续监控 JSON，变化时触发评估并写入 result.json
# ------------------------------------------------------------------ 
